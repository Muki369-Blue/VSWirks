...
const {
  ...
  resolveModelForTask,
  resolveModelRole
} = require("./controller/model-roles.cjs");
...
class VSWirksController {
  ...
  // ── Ollama, Git, Media, Model Lab methods → see controller/ mixins
  ...
  async handle(channel, payload) {
    // IPC boundary validation: reject non-object payloads (except null/undefined)
    if (payload !== null && payload !== undefined && typeof payload !== "object") {
      return { ok: false, error: "Invalid payload: expected object" };
    }
    try {
      return await this._dispatch(channel, payload);
    } catch (error) {
