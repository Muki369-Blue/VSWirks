...
class NativeChatIntegration {
  ...
  async runAgentRequest(commandSpec, model, messages, response, token) {
    ...
    for (let step = 0; step < maxSteps; step += 1) {
      ...
      if (!toolCalls.length) {
        if (executionMode === "act") {
          const materialized = await tryMaterializeActReply({
            model,
            messages,
            assistantContent: finalText,
            fetchJson: (payload) => fetchRuntimeJson("/chat/completions", payload, token),
            executeTool: (toolCall) => executeWorkspaceToolCall(toolCall),
          ...
          if (materialized.applied) {
            for (const path of materialized.touchedPaths) {
              touchedPaths.add(path);
            }
            if (materialized.summary.trim()) {
              response.markdown(materialized.summary);
            }
            if (finalUsage) {
              response.markdown(
      ...
      if (!toolCalls.length) {
        if (executionMode === "act") {
          const materialized = await tryMaterializeActReply({
            ...
            messages,
            assistantContent: finalText,
            fetchJson: (payload) => this.fetchJson("/chat/completions", payload),
            executeTool: (toolCall) => this.executeToolCall(toolCall),
            onToolResult: (toolName, result) => {
          ...
          if (materialized.applied) {
            this.finalizeAssistantPlaceholder(assistantId, {
              content: materialized.summary || finalText || "(materialized scaffold)",
              usage: finalUsage
            });
            this.lastStatus = "Agent complete";
            return;
          }
        }
...
async function tryMaterializeActReply({
  ...
  if (!workspaceSnapshot || !workspaceSnapshot.empty) {
    return { applied: false, touchedPaths: [], summary: "" };
  }
  ...
  const touchedPaths = [];
  let deniedCount = 0;
  const limitedFiles = manifest.files.slice(0, 40);

  for (const file of limitedFiles) {
    const toolCall = createSyntheticToolCall("write_file", {
      path: file.path,
  ...
  summaryLines.push(
    manifest.summary && manifest.summary.trim()
      ...
      : `Materialized ${touchedPaths.length} files into the workspace.`
  );
  if (deniedCount) {
    ...
    messages: [
      {
        ...
        content:
          ...
          '{"summary":"short summary","files":[{"path":"relative/path","content":"complete file contents"}],"open":["optional/relative/path"]}. ' +
          "Do not use markdown fences. Keep paths relative. Include complete production-ready contents."
      },
      {
        role: "user",
        content:
          "The workspace is empty. Convert the requested repository into concrete files for this workspace. " +
      ...
      },
      {
        role: "user",
        content:
          "Return the JSON object now. Include a practical README, project metadata, source files, and tests. " +
          "If folders are needed, encode them through file paths."
      }
    ]
  };
...
function parseMaterializationManifest(text) {
  ...
  for (const candidate of candidates) {
    ...
    const files = parsed.files
      ...
      .filter((entry) => entry.path && entry.content !== undefined);
...
function normalizeOpenTargets(openTargets, touchedPaths) {
  const normalized = Array.isArray(openTargets)
    ? openTargets.map((item) => String(item || "").trim()).filter(Boolean)
    : [];
  if (normalized.length) {
    return normalized;
  }

  const preferred = touchedPaths.find((item) => /(^|\/)README(\.[^/]+)?$/i.test(item));
  if (preferred) {
    return [preferred];
  }

  return touchedPaths.length ? [touchedPaths[0]] : [];
}
...
function createSyntheticToolCall(name, args) {
  return {
    id: makeId("toolcall"),
    function: {
      name,
